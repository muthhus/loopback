...
Registry.prototype.configureModel = function(ModelCtor, config) {
  ...
  var hasNewMethods = config.methods && Object.keys(config.methods).length;
  var hasDescendants = this.getModelByType(ModelCtor) !== ModelCtor;
  if (hasNewMethods && hasDescendants) {
    g.warn(
      'Child models of `%s` will not inherit newly defined remote methods %j.',
      modelName, config.methods);
  }
